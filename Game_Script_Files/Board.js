function Board(CSize, cells, mode, difficulty) {
    ...
    for (var i = 0; i < this.cells; i++) {
      for (var j = 0; j < this.cells; j++) {
        ...
        if (this.board[i][j].input == "X") {
          ...
          line(
            this.board[i][j].x + offset,
            this.board[i][j].y + offset,
            this.board[i][j].x + this.board[i][j].s - offset,
            this.board[i][j].y + this.board[i][j].s - offset
          );
          line(
            this.board[i][j].x + this.board[i][j].s - offset,
            this.board[i][j].y + offset,
  ...
  this.turn = function () {
    if (this.mode === 'human-computer' && this.index === 1) {
      return; // Ignore human input when it's the computer's turn
    }
    for (var i = 0; i < this.cells; i++) {
      for (var j = 0; j < this.cells; j++) {
        if (this.Onit(this.board[i][j])) {
...
function setup() {
  ...
  button_add = createButton("Add Cell")
    .style("border-radius", "50px")
    .style("background-color", "coral");
  button_sub = createButton("Sub Cell")
    .style("border-radius", "50px")
    .style("background-color", "coral");
  ...
  h3 = createElement("h3", "This is a Simple Tic-Tac-Toe game made with JavaScript (p5). (Press 'r/R' or 'Reset' button to reset the game) v1.0")
    .style("color", "White")
...
function mousePressed() {
  if (gameMode === "human-computer" && board.index === 1) {
    return; // Ignore human input when it's the computer's turn
  }
  board.turn();
}
